          ...
          {isComplete ? (
            ...
            <div className="border border-solid border-[rgba(42,74,67,0.4)] rounded-md bg-[rgba(255,255,255,0.02)] my-[12px] mx-0 px-[16px] py-[20px]">
              ...
              <div className="text-[11px] leading-[1.4] mb-[10px] text-[var(--color-text-dim)]">
                Zur Vermeidung irreführender Prozentanteile wird der Ring erst gezeichnet, wenn alle
                4 Akquisitionsquellen vorliegen.
              </div>
              {missingIds.length > 0 && (
          ...
          <table className="w-full border-collapse text-[var(--color-text-muted)]">
            ...
            <tbody>
              {ARR_MIX_IDS.map((id) => {
                ...
                if (item) {
                  const pct =
                    isComplete && totalArr > 0 ? ((item.value / totalArr) * 100).toFixed(1) : null;
                  return (
                    <tr
                      key={id}
                    ...
                    >
                      ...
                      <td
                        className={`text-right py-[3px] px-0 ${item.qualityStatus === 'degraded' ? 'text-[#ff7a3d]' : 'text-[#00f2fe]'}`}
                      >
