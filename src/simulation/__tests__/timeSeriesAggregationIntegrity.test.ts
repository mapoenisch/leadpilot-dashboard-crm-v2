...
import { runTimeSeriesTailChecks } from './timeSeriesIntegrityTail.test';
...
export async function runTimeSeriesAggregationTest(): Promise<{ success: boolean; log: string[] }> {
  ...
  function createMockRun(
    ...
    const finalTs =
      timeSeries && timeSeries.length > 0 ? timeSeries[timeSeries.length - 1] : undefined;
    return {
      runId,
      scenarioId: 'scen-test',
      scenarioVersionId: 'ver-test-v1',
  ...
  // ---------------------------------------------------------
  log.push('\n--- TEST A: Einzelner Tick Aggregation & Quantile ---');
  const tsSingleTickRun1: TimeSeriesPoint[] = [
    {
      tick: 0,
      dayIndex: 0,
      simulatedDate: '01.01.2026',
      metrics: { arr: 100000, mrr: 8333, customers: 10, wonDeals: 2 },
    },
  ];
  const tsSingleTickRun2: TimeSeriesPoint[] = [
    {
      tick: 0,
      dayIndex: 0,
      simulatedDate: '01.01.2026',
      metrics: { arr: 200000, mrr: 16666, customers: 20, wonDeals: 4 },
    },
  ];
  const tsSingleTickRun3: TimeSeriesPoint[] = [
    {
      tick: 0,
      dayIndex: 0,
      simulatedDate: '01.01.2026',
      metrics: { arr: 300000, mrr: 25000, customers: 30, wonDeals: 6 },
    },
  ];
  ...
  if (testAPassed) {
    log.push(
      `✅ TEST A PASSED: Single tick computed exact stats (Median: ${tsPointA?.metrics.arr.median} €, P10: ${tsPointA?.metrics.arr.p10} €, P90: ${tsPointA?.metrics.arr.p90} €).`,
    );
  } else {
  ...
  const tsMultiTickRun1: TimeSeriesPoint[] = [
    {
      ...
      dayIndex: 0,
      simulatedDate: '01.01.2026',
      metrics: { arr: 100000, mrr: 8333, customers: 10, wonDeals: 1 },
    },
    {
      tick: 1,
      dayIndex: 1,
      simulatedDate: '02.01.2026',
      metrics: { arr: 150000, mrr: 12500, customers: 15, wonDeals: 2 },
    },
  ];
  const tsMultiTickRun2: TimeSeriesPoint[] = [
    {
      ...
      dayIndex: 0,
      simulatedDate: '01.01.2026',
      metrics: { arr: 200000, mrr: 16666, customers: 20, wonDeals: 3 },
    },
    {
      tick: 1,
      dayIndex: 1,
      simulatedDate: '02.01.2026',
      metrics: { arr: 250000, mrr: 20833, customers: 25, wonDeals: 5 },
    },
  ];
  ...
  if (testBPassed) {
    log.push(
      `✅ TEST B PASSED: Tick 0 (Median 150.000 €) and Tick 1 (Median 200.000 €) aggregated separately without cross-tick pollution.`,
    );
  } else {
  ...
  const testCPassed = resB.metrics.timeSeries?.every(
    (pt) =>
      pt.metrics.arr.p10 <= pt.metrics.arr.median && pt.metrics.arr.median <= pt.metrics.arr.p90,
  );
  ...
  if (testDPassed) {
    log.push(
      `✅ TEST D PASSED: Percentiles match exact linear interpolation formula (P10: ${p10Calc}, P50: ${p50Calc}, P90: ${p90Calc}).`,
    );
  } else {
    log.push(
      `❌ TEST D FAILED: Linear interpolation error! Got P10: ${p10Calc}, P50: ${p50Calc}, P90: ${p90Calc}`,
    );
    overallPassed = false;
  }
  ...
  const testEPassed =
    resE.validRunCount === 1 && resE.metrics.timeSeries?.[0]?.metrics.arr.median === 100000;

  if (testEPassed) {
    log.push(
      '✅ TEST E PASSED: Only 1 COMPLETED run processed (RUNNING, CANCELLED, FAILED excluded).',
    );
  } else {
  ...
  const testFPassed =
    tsB0?.tick === 0 &&
    tsB1?.tick === 1 &&
    tsB0?.simulatedDate === '01.01.2026' &&
    tsB1?.simulatedDate === '02.01.2026';
  if (testFPassed) {
    log.push(
      '✅ TEST F PASSED: Ticks are strictly aligned tick-synchronously (01.01.2026 -> 02.01.2026).',
    );
  } else {
  ...
  log.push('\n--- TEST G: AggregatedMetrics.timeSeries Befüllung ---');
  const testGPassed =
    Array.isArray(resB.metrics.timeSeries) && resB.metrics.timeSeries.length === 2;
  if (testGPassed) {
    log.push(
      `✅ TEST G PASSED: AggregatedMetrics.timeSeries correctly populated with ${resB.metrics.timeSeries?.length} points.`,
    );
  } else {
  ...
  if (testHPassed) {
    log.push(
      '✅ TEST H PASSED: All 4 V1 KPIs (ARR, MRR, Customers, WonDeals) present in time series metrics.',
    );
  } else {
  ...
  if (testIPassed) {
    log.push(
      `✅ TEST I PASSED: Mean (${ptI?.mean}) and Sample StdDev (${ptI?.stdDev}) calculated correctly.`,
    );
  } else {
    log.push(
      `❌ TEST I FAILED: Mean/StdDev calculation error! Got Mean: ${ptI?.mean}, StdDev: ${ptI?.stdDev}`,
    );
    overallPassed = false;
  }
  ...
  if (testJPassed) {
    log.push(
      `✅ TEST J PASSED: Min (${ptI?.min} €) and Max (${ptI?.max} €) bounds captured correctly.`,
    );
  } else {
  ...
  log.push('\n--- TEST K: Endwertaggregation Regressionsschutz ---');
  const testKPassed =
    resB.metrics.arr.median === 200000 &&
    resB.metrics.arr.p10 === 160000 &&
    resB.metrics.arr.p90 === 240000;
  if (testKPassed) {
    log.push(
      `✅ TEST K PASSED: Existing end-of-run aggregation remains 100% accurate (End Median ARR: ${resB.metrics.arr.median} €).`,
    );
  } else {
    log.push(
      `❌ TEST K FAILED: End-of-run aggregation regression detected! Got P10: ${resB.metrics.arr.p10}, P50: ${resB.metrics.arr.median}, P90: ${resB.metrics.arr.p90}`,
    );
    overallPassed = false;
  }
  ...
  const testLPassed =
    resL.validRunCount === 1 && resL.metrics.timeSeries?.[0]?.metrics.arr.median === 100000;
  if (testLPassed) {
  ...
  if (testMPassed) {
    log.push(
      '✅ TEST M PASSED: Incompatible tick alignment (Tick 1 vs Tick 5) correctly rejected with INCOMPATIBLE_TIMESERIES error.',
    );
  } else {
  ...
  if (testNPassed) {
    log.push(
      '✅ TEST N PASSED: ScenarioService.getScenarioAggregation() exposes scenario aggregation result.',
    );
  } else {
  ...
  const testOPassed =
    ...
    typeof scenAggregation.metrics.arr.p10 === 'number';
  if (testOPassed) {
    log.push(
      '✅ TEST O PASSED: React UI components perform 0 quantile or percentile calculations.',
    );
  } else {
  ...
  if (testPPassed) {
    log.push(
      '✅ TEST P PASSED: React UI components access simulation data exclusively via Application Service / Context.',
    );
  } else {
  ...
  if (testQPassed) {
    log.push(
      '✅ TEST Q PASSED: DetailTierView limits individual run overlay badges to maximum 5 runs.',
    );
  } else {
  ...
  // 067K / G57: Schlussteil (TEST R–X) in timeSeriesIntegrityTail.test.ts —
  // gleiche Reihenfolge, gleiche Logs.
  const tailPassed = await runTimeSeriesTailChecks(log, scenAggregation, runA1, runA2, runA3);
