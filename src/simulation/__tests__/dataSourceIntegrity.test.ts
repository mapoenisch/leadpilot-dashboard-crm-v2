...
export async function runDataSourceTest(): Promise<{ success: boolean; log: string[] }> {
  ...
  log.push(
    '=== STARTING AUFTRAG 016 TEST SUITE (DATA SOURCE ABSTRACTION & BASELINE SNAPSHOTS) ===',
  );
  ...
  ok =
    a(
      log,
      'simulated-crm registriert',
      dataSourceRegistry.list().some((s) => s.id === 'simulated-crm'),
    ) && ok;

  // 2. Baseline capture & counts check
  const snap = await BaselineSnapshotService.capture(
    'simulated-crm',
    'test-v1',
    '2026-01-01',
  ...
  ok =
    a(
      log,
      'counts (20 Companies, 100 Contacts, 40 Deals)',
      snap.counts.companies === 20 && snap.counts.contacts === 100 && snap.counts.deals === 40,
    ) && ok;
  ...
  ok =
    a(
      log,
      'contact->company refs',
      snap.contacts.every((c) => companyIds.has(c.companyId)),
    ) && ok;

  // 4. Frozen snapshot retrieval
  ok =
    a(log, 'frozen abrufbar', BaselineSnapshotService.get('test-v1').version === 'test-v1') && ok;
  ...
  const s2 = await BaselineSnapshotService.capture(
    'simulated-crm',
    'test-v2',
    '2026-01-01',
    '2026-01-01T00:00:00.000Z',
  ...
  ok =
    a(
      log,
      'capture deterministisch',
      JSON.stringify(snap.companies) === JSON.stringify(s2.companies),
    ) && ok;
  ...
  ok =
    a(
      log,
      'baseline-file:2026-08-31-v1 registriert',
      dataSourceRegistry.list().some((s) => s.id === 'baseline-file:2026-08-31-v1'),
    ) && ok;
  ok =
    a(
      log,
      'baseline-file:2026-09-15-v2 registriert',
      dataSourceRegistry.list().some((s) => s.id === 'baseline-file:2026-09-15-v2'),
    ) && ok;
  const fileV1 = await dataSourceRegistry.get('baseline-file:2026-08-31-v1').fetchSnapshot();
  ok =
    a(
      log,
      'baseline-file v1 daten intakt (20/100/40)',
      fileV1.companies.length === 20 &&
        fileV1.contacts.length === 100 &&
        fileV1.deals.length === 40,
    ) && ok;
  const fileV2 = await dataSourceRegistry.get('baseline-file:2026-09-15-v2').fetchSnapshot();
  ok =
    a(
      log,
      'baseline-file v2 daten intakt (20/100/40)',
      fileV2.companies.length === 20 &&
        fileV2.contacts.length === 100 &&
        fileV2.deals.length === 40,
    ) && ok;
  ...
  ok =
    a(
      log,
      'CRMRepository.getCompanies() liefert 20 Companies aus aktiver DataSource',
      comps.length === 20,
    ) && ok;
  ...
  ok =
    a(
      log,
      'Pluggable source registration succeeds without breaking code',
      dataSourceRegistry.list().some((s) => s.id === 'hubspot-test-stub'),
    ) && ok;
