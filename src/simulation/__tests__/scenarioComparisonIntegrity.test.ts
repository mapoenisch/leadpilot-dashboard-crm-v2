...
import { runScenarioComparisonTailChecks } from './scenarioComparisonTail.test';

export async function runScenarioComparisonIntegrityTest(): Promise<{
  success: boolean;
  log: string[];
}> {
  const log: string[] = [];
  log.push(
    '=== STARTING AUFTRAG 014 TEST SUITE (SCENARIO COMPARISON, GOAL TARGET EVALUATOR & MULTI-RUN ANALYTICS) ===',
  );
  ...
  const testBPassed =
    arrDef.direction === 'HIGHER_IS_BETTER' && ebitdaDef.direction === 'HIGHER_IS_BETTER';
  ...
  const testCPassed =
    cacDef.direction === 'LOWER_IS_BETTER' && csQueueDef.direction === 'LOWER_IS_BETTER';
  ...
  if (testDPassed) {
    log.push(
      `✅ TEST D PASSED: Status ACHIEVED evaluated correctly (${resD.achievementPercent}% achieved).`,
    );
  } else {
    log.push(
      `❌ TEST D FAILED: Status ACHIEVED mismatch (${resD.status}, ${resD.achievementPercent}%)`,
    );
    overallPassed = false;
  }
  ...
  if (testEPassed) {
    log.push(
      `✅ TEST E PASSED: Status AT_RISK evaluated correctly (${resE.achievementPercent}% achieved).`,
    );
  } else {
    log.push(
      `❌ TEST E FAILED: Status AT_RISK mismatch (${resE.status}, ${resE.achievementPercent}%)`,
    );
    overallPassed = false;
  }
  ...
  if (testFPassed) {
    log.push(
      `✅ TEST F PASSED: Status MISSED evaluated correctly (${resF.achievementPercent}% achieved).`,
    );
  } else {
    log.push(
      `❌ TEST F FAILED: Status MISSED mismatch (${resF.status}, ${resF.achievementPercent}%)`,
    );
    overallPassed = false;
  }
  ...
  if (testGPassed) {
    log.push(
      '✅ TEST G PASSED (Decision 1313): Undefined target returned NO_TARGET without fake probabilities.',
    );
  } else {
    log.push(
      '❌ TEST G FAILED: Fake probabilities or wrong status generated for undefined target!',
    );
    overallPassed = false;
  }
  ...
  log.push('\n--- TEST H: Structured explanation string generated (Decision 1320) ---');
  const testHPassed =
    resE.explanation.includes('Ziel gefährdet') && resE.explanation.includes('88%');

  if (testHPassed) {
    log.push(
      `✅ TEST H PASSED (Decision 1320): Explanation string generated (${resE.explanation}).`,
    );
  } else {
  ...
  const testLPassed =
    !isNaN(compL.percentChange) && isFinite(compL.percentChange) && compL.percentChange === 100;

  if (testLPassed) {
    log.push('✅ TEST L PASSED: Division by 0 baseline handled safely (percentChange = 100%).');
  } else {
    log.push(
      `❌ TEST L FAILED: Division by zero produced NaN or Infinity (${compL.percentChange})`,
    );
    overallPassed = false;
  }
  ...
  if (testMPassed) {
    log.push(
      '✅ TEST M PASSED (Decisions 1300–1301): Multi-run selector capped strictly at 5 runs.',
    );
  } else {
  ...
  if (testNPassed) {
    log.push(
      '✅ TEST N PASSED (Decision 1303): Selected individual runs visually separated from Quantile bands.',
    );
  } else {
  ...
  // ---------------------------------------------------------
  log.push('\n--- TEST Q: 0 Math.random() / Date.now() in Goal Target domain ---');
  const evalQ1 = GoalTargetEvaluator.evaluateGoalTarget(
    'liveARR',
    500000,
    { kpiId: 'liveARR', targetValue: 500000 },
    10,
  );
  const evalQ2 = GoalTargetEvaluator.evaluateGoalTarget(
    'liveARR',
    500000,
    { kpiId: 'liveARR', targetValue: 500000 },
    10,
  ...
  const testQPassed =
    evalQ1.status === evalQ2.status &&
    evalQ1.achievementPercent === evalQ2.achievementPercent &&
    evalQ1.explanation === evalQ2.explanation;
  ...
  // ---------------------------------------------------------
  log.push('\n--- TEST S: Financial metrics from Auftrag 011 preserved ---');
  const ebitdaGoalS = GoalTargetEvaluator.evaluateGoalTarget(
    'ebitda',
    60000,
  ...
  const testSPassed =
    ebitdaGoalS.status === 'ACHIEVED' && (ebitdaGoalS.achievementPercent ?? 0) === 120;
  ...
  // ---------------------------------------------------------
  log.push('\n--- TEST T: State Machine & Invariants from Auftrag 012 preserved ---');
  const csQueueGoalT = GoalTargetEvaluator.evaluateGoalTarget(
    'csQueueTime',
    2,
    { kpiId: 'csQueueTime', targetValue: 3 },
    50,
  );
  ...
  // ---------------------------------------------------------
  log.push('\n--- TEST U: SnapshotPruningManager from Auftrag 013 preserved ---');
  const goalU = GoalTargetEvaluator.evaluateGoalTarget(
    'cac',
    400,
    { kpiId: 'cac', targetValue: 500 },
    30,
  );
  ...
  const evalV = GoalTargetEvaluator.evaluateGoalTarget(
    'liveARR',
    mockAggregationV.metrics.arr.median,
    { kpiId: 'liveARR', targetValue: 500000 },
    90,
  );
  ...
  if (testVPassed) {
    log.push(
      '✅ TEST V PASSED: GoalTargetEvaluator processes Monte Carlo aggregated metrics seamlessly.',
    );
  } else {
  ...
  const compW = GoalTargetEvaluator.computeBaselineComparison(
    'liveARR',
    mockAggregationV.metrics.arr.median,
    411840,
  );
  const testWPassed =
    typeof compW.absoluteDelta === 'number' &&
    typeof compW.percentChange === 'number' &&
    compW.isPositiveChange === true;
  ...
  const { DEFAULT_BASE_2026_SCENARIO_ID, DEFAULT_BASE_2026_VERSION_ID } =
  ...
  if (testYPassed) {
    log.push(
      '✅ TEST Y PASSED: ScenarioService.compareVersions detected parameter changes and deltas accurately.',
    );
  } else {
  ...
  const testZPassed =
    Boolean(
      arrKpiComp &&
      arrKpiComp.direction === 'HIGHER_IS_BETTER' &&
      arrKpiComp.baselineValue === 411840,
    ) &&
    Boolean(
      customersKpiComp &&
      customersKpiComp.direction === 'HIGHER_IS_BETTER' &&
      customersKpiComp.baselineValue === 66,
    );
  ...
  // 067K / G57: Schlussteil (TEST AB–X) in scenarioComparisonTail.test.ts —
  // gleiche Reihenfolge, gleiche Logs.
  const tailPassed = await runScenarioComparisonTailChecks(log, diffResult, ver2);
