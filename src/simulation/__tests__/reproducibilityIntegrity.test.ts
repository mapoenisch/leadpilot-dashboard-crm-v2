...
export async function runReproducibilityTest(): Promise<{ success: boolean; log: string[] }> {
  ...
  try {
    ...
    ok =
      assert(
        log,
        'Manifest identical across repeated runs',
        JSON.stringify(A.run.manifest) === JSON.stringify(B.run.manifest),
      ) && ok;

    // 2) Event-Stream identisch (id, type, correlationId, Reihenfolge)
    const sig = (r: typeof A) =>
      r.events.map((e) => `${e.id}|${e.type}|${e.correlationId}`).join('\n');
    ok =
      assert(
        log,
        'Event-Stream identical (id, type, correlationId, sequence)',
        sig(A) === sig(B),
      ) && ok;

    // 3) End-Snapshot / RNG-State identisch
    ok =
      assert(log, 'End RNG-State identical across runs', A.run.rngState === B.run.rngState) && ok;
    ok =
      assert(
        log,
        'Final Metrics identical across runs',
        JSON.stringify(A.run.finalMetrics) === JSON.stringify(B.run.finalMetrics),
      ) && ok;
    ...
    ok =
      assert(
        log,
        'simulationStartDate deterministic (2026-01-01)',
        A.run.manifest.simulationStartDate === '2026-01-01',
      ) && ok;
    ...
    ok =
      assert(
        log,
        'Aktive Datenquelle umgestellt auf baseline-file:2026-09-15-v2',
        dataSourceRegistry.getActive().info.id === 'baseline-file:2026-09-15-v2',
      ) && ok;
    ...
    ok =
      assert(
        log,
        'reproduce(originalRunId) bindet weiterhin die ursprüngliche baselineVersion',
        repro.run.manifest.baselineVersion === A.run.manifest.baselineVersion,
      ) && ok;
    ok =
      assert(
        log,
        'reproduce(originalRunId) liefert identischen End-RNG-State trotz geänderter aktiver DataSource',
        repro.run.rngState === A.run.rngState,
      ) && ok;
    ok =
      assert(
        log,
        'reproduce(originalRunId) liefert identische Final Metrics',
        JSON.stringify(repro.run.finalMetrics) === JSON.stringify(A.run.finalMetrics),
      ) && ok;
    ...
    ok =
      assert(
        log,
        'Nicht verfügbare Baseline-Version löst kontrolliert DataSourceError("UNKNOWN_SOURCE") aus',
        threwUnknown,
      ) && ok;
  } catch (err) {
    log.push(
      `❌ Unexpected error in Reproducibility Test: ${err instanceof Error ? err.message : (err as { message: string }).message}`,
    );
    ok = false;
  } finally {
