...
export async function runResourceInfrastructureIntegrityTest(): Promise<{
  success: boolean;
  log: string[];
}> {
  const log: string[] = [];
  ...
  if (allPathsValid && totalAssetCount === 35) {
    log.push(
      `✅ TEST C PASSED: All ${totalAssetCount} resource asset paths mapped consistently under /resources/.`,
    );
  } else {
    log.push(
      `❌ TEST C FAILED: Asset path mapping invalid or count mismatch (${totalAssetCount} vs 35).`,
    );
    overallPassed = false;
  }
  ...
  if (countsMatch) {
    log.push(
      '✅ TEST D PASSED: Exact page and slide counts verified for all 8 resources (Total 35 pages/slides/assets).',
    );
  } else {
  ...
  if (
    ...
    log.push(
      '✅ TEST G PASSED: ResourceRegistry helper filters (Category & Type) operate accurately (Mkt: 4, Sales: 3, Ops: 1; Decks: 3, Docs: 3, HTML: 1, Graphic: 1).',
    );
  } else {
  ...
  if (crmValid) {
    log.push(
      '✅ TEST H PASSED: Historical Ebene A CRM baseline remains 100% pristine (20 Companies, 100 Contacts, 40 Deals).',
    );
  } else {
