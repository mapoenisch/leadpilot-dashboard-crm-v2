...
import { CHART_ERLOESE, CHART_KOSTEN, CHART_MRR26, CHART_CHURN26, CHART_BUDGET } from "../../domain/finanzenData";
...
export async function runReconstructedChartsIntegrityTest(): Promise<{ success: boolean; log: string[] }> {
  const log: string[] = [];
  ...
  const all15Present = chartKeys.length === 15 && chartKeys.every((k) => charts[k] && ["line", "bar", "doughnut"].includes(charts[k].type));
  if (all15Present) {
  ...
  for (const [id, chart] of Object.entries(charts)) {
    ...
    for (const ds of chart.datasets) {
      if (ds.data.length !== labelCount) {
        aligned = false;
        log.push(`❌ Alignment error in ${id}: ${ds.data.length} data points vs ${labelCount} labels`);
      }
    }
  }
  if (aligned) {
  ...
  const testCSuccess =
    ...
    CHART_SEGMENT.datasets[0].data[0] === 178560 && // Maschinenbau 178.560 €
    BRAND.chart.datasets[0].data[3] === 2900 && // Web-Besucher Q4: 2.900
    PLANUNG.chartPlanbudget.datasets[0].data[0] === 2500 && // Aug 26: 2.500 €
    KAMPAGNE.chartKampbudget.datasets[0].data[0] === 1000 && // Paid Social: 1.000 €
    CHART_ERLOESE.datasets[0].data[0] === 307600 && // Abo-Umsatz: 307.600 €
    CHART_KOSTEN.datasets[1].data[0] === 336 && // Umsatz FY25: 336 k€
    CHART_MRR26.datasets[0].data[4] === 51667 && // MRR Dez 26: 51.667 €
    CHART_CHURN26.datasets[0].data[3] === 1.8 && // Churn Q4 26: 1.8%
    CHART_BUDGET.datasets[0].data[0] === 600000 && // Personal 600k
    CHART_OKR.datasets[1].data[0] === 62.0 && // ARR Ziel 2026: 62.0
    CHART_TREIBER.datasets[0].data[0] === 72000; // Trial-to-Paid: +72.000 €

  if (testCSuccess) {
    log.push("✅ TEST C PASSED: Exact numerical values verified against historical reference specifications.");
  } else {
  ...
  log.push("\n--- TEST D: Multi-Dataset Integrity ---");
  const multiDatasetCharts = ["c-produkt", "c-brand", "c-plankpi", "c-kosten", "c-mrr26", "c-churn26", "c-okr"];
  const allMultiValid = multiDatasetCharts.every((id) => charts[id].datasets.length >= 2);
  if (allMultiValid) {
    log.push(`✅ TEST D PASSED: Multi-dataset comparative charts valid (${multiDatasetCharts.length} multi-series charts).`);
  } else {
  ...
  for (const id of doughnutCharts) {
    const total = charts[id].datasets[0].data.reduce((a: number, b: number) => a + b, 0);
  ...
  for (const chart of Object.values(charts)) {
    for (const ds of chart.datasets) {
      if (ds.color && !ds.color.startsWith("#") && !ds.color.startsWith("var(")) colorsValid = false;
    }
  }
  if (colorsValid) {
    log.push("✅ TEST F PASSED: All chart datasets styled using LeadPilot brand colors and design tokens.");
  } else {
  ...
  if (crmValid) {
    log.push("✅ TEST G PASSED: Historical Ebene A CRM baseline remains 100% pristine (20 Companies, 100 Contacts, 40 Deals).");
  } else {
  ...
  if (resList.length === 8) {
    log.push("✅ TEST H PASSED: Auftrag 015 Resource Registry remains 100% intact (8 Resources, 35 Assets).");
  } else {
