...
export async function runMultiScenarioComparisonTest(): Promise<boolean> {
  ...
  const v2 = scenService.createScenarioVersion(
    testScenario.id,
    {
      salesRepCount: 5,
  ...
  const v3 = scenService.createScenarioVersion(
    testScenario.id,
    {
      salesRepCount: 2,
  ...
  const v4 = scenService.createScenarioVersion(
    testScenario.id,
    {
      salesRepCount: 3,
  ...
  if (res3.parameterMatrix.length < 6) {
    throw new Error(
      `TEST 1 FAILED: Expected full parameter matrix, got ${res3.parameterMatrix.length}`,
    );
  }
  if (res3.kpiMatrix.length < 7) {
    throw new Error(
      `TEST 1 FAILED: Expected full KPI matrix (7+ metrics), got ${res3.kpiMatrix.length}`,
    );
  }
  ...
  const expectedDimensions: TradeOffDimension[] = [
    'GROWTH',
    'PROFITABILITY',
    'LIQUIDITY',
    'ACQUISITION',
  ...
  for (const tradeOff of res4.tradeOffs) {
    ...
    if (evalKeys.length !== 4) {
      throw new Error(
        `TEST 2 FAILED: Dimension ${tradeOff.dimension} missing evaluation for all 4 versions.`,
      );
    }
  }
  ...
  if (res4Record.compositeScore || res4Record.overallRank || res4Record.winnerScore) {
    throw new Error(
      'TEST 2 FAILED: Unlawful artificial composite score detected in comparison result.',
    );
  }
  logger.info(
    '✅ TEST 2 PASSED: 5-dimension trade-off analysis valid without artificial composite score.',
  );
  ...
  logger.info(
    '✅ TEST 3 PASSED: Automated root-cause differences identified with domain explanations.',
  );
  ...
  const adoptedVersion = scenService.adoptConfiguration(
    v2.id,
    testScenario.id,
    'Adopted v2 for production trial',
  );
  if (!adoptedVersion) {
    throw new Error('TEST 4 FAILED: adoptConfiguration returned null.');
  }
  if (
    adoptedVersion.parameters.salesRepCount !== 5 ||
    adoptedVersion.parameters.marketingBudgetYearly !== 140000
  ) {
  ...
  if (!errCaughtNotFound) {
    throw new Error(
      'TEST 5 FAILED: compareMultipleVersions did not throw NOT_FOUND for unknown version.',
    );
  }
  logger.info(
    '✅ TEST 5 PASSED: Strict boundary constraints (2 <= n <= 4) and error handling enforced.',
  );
  ...
  if (arrRow.deltasAgainstRef[v1.id] !== 0) {
    throw new Error(
      `TEST 6 FAILED: Expected delta against self = 0, got ${arrRow.deltasAgainstRef[v1.id]}`,
    );
  }
  logger.info(
    '✅ TEST 6 PASSED: Relative deltas and directional favorability computed accurately.',
  );
