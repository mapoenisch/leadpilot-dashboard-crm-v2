...
export async function runRegionSplitIntegrityTest(): Promise<{ success: boolean; log: string[] }> {
  ...
  if (hasValidStructure) {
    log.push("✅ TEST A PASSED: REGIONEN object contains valid rows array and distribution mapping.");
  } else {
  ...
  if (de === 61 && at === 3 && ch === 2 && total === 66) {
    log.push(`✅ TEST B PASSED: Exact region distribution verified: Deutschland=${de}, Österreich=${at}, Schweiz=${ch}, Total=${total}.`);
  } else {
    log.push(`❌ TEST B FAILED: Region distribution mismatch (DE=${de}, AT=${at}, CH=${ch}, Total=${total}).`);
    overallPassed = false;
  }

  // --- TEST C: Region values are strictly positive (> 0) ---
  log.push("\n--- TEST C: Region values are strictly positive (> 0) ---");
  if (de > 0 && at > 0 && ch > 0 && deRow?.kunden === 61 && atRow?.kunden === 3 && chRow?.kunden === 2) {
    log.push("✅ TEST C PASSED: All regional customer values are strictly positive and consistent in table rows.");
  } else {
  ...
  const execKundenKpi = EXEC_KPIS_1.find((k) => k.label.includes("Kunden") || k.label.includes("Aktive Kunden"));
  const execKundenValue = execKundenKpi ? parseInt(execKundenKpi.value, 10) : 0;

  if (calculatedSum === 66 && calculatedSum === execKundenValue) {
    log.push(`✅ TEST D PASSED: Regional customer sum (${calculatedSum}) equals EXEC_KPIS customer count (${execKundenValue}).`);
  } else {
    log.push(`❌ TEST D FAILED: Regional sum (${calculatedSum}) does not match EXEC_KPIS (${execKundenValue}).`);
    overallPassed = false;
  }
  ...
  const containsPreciseInfo = val.includes("Deutschland 61") && val.includes("Österreich 3") && val.includes("Schweiz 2");
  if (containsPreciseInfo) {
    log.push(`✅ TEST E PASSED: ICP_SPECS region explicitly documents current customer count: "${val}"`);
  } else {
    log.push(`❌ TEST E FAILED: ICP_SPECS region does not contain precise customer numbers: "${val}"`);
    overallPassed = false;
  }
  ...
  if (crmValid) {
    log.push("✅ TEST F PASSED: Historical Ebene A CRM baseline remains 100% pristine (20 Companies, 100 Contacts, 40 Deals).");
  } else {
    log.push(`❌ TEST F FAILED: CRM Baseline mutated (${audit.companiesValid}/${audit.contactsValid}/${audit.dealsValid})`);
    overallPassed = false;
  }
