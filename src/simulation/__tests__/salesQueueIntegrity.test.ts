...
import { createInitialState, runSalesQueueTailChecks } from './salesQueueIntegrityTail.test';

export async function runSalesQueueIntegrityTest(): Promise<{ success: boolean; log: string[] }> {
  const log: string[] = [];
  log.push(
    '=== STARTING AUFTRAG 009 TEST SUITE (SALES CAPACITY, WORKLOAD & SALES QUEUE MODEL) ===',
  );
  ...
  if (testAPassed) {
    log.push(
      `✅ TEST A PASSED: Sales capacity derived deterministically from salesRepCount (2 FTE -> ${cap1}, 5 FTE -> ${cap2}).`,
    );
  } else {
  ...
  if (testBPassed) {
    log.push(
      `✅ TEST B PASSED: Sufficient capacity (5 FTE for 1 lead) resulted in 0 waiting and 1 in-progress.`,
    );
  } else {
    log.push(
      `❌ TEST B FAILED: Unjustified queue created! Waiting: ${projB.waitingCount}, InProgress: ${projB.inProgressCount}`,
    );
    overallPassed = false;
  }
  ...
  const testCPassed =
    projC.availableCapacity === 1 && projC.inProgressCount === 1 && projC.waitingCount === 2;
  if (testCPassed) {
    log.push(
      `✅ TEST C PASSED: Insufficient capacity (1 FTE for 3 leads) created 1 in-progress and 2 waiting items.`,
    );
  } else {
    log.push(
      `❌ TEST C FAILED: Queue creation error! Waiting: ${projC.waitingCount}, InProgress: ${projC.inProgressCount}`,
    );
    overallPassed = false;
  }
  ...
  if (testDPassed) {
    log.push(
      '✅ TEST D PASSED: Reached dueAt without capacity did NOT auto-complete; item remained WAITING and queueTicks increased.',
    );
  } else {
  ...
  if (testFPassed) {
    log.push(
      `✅ TEST F PASSED: processTicks (${processedF?.processTicks}) and queueTicks (${processedF?.queueTicks}) stored separately.`,
    );
  } else {
  ...
  const testGPassed =
    processedF?.totalSalesCycleTicks ===
    (processedF?.processTicks ?? 0) + (processedF?.queueTicks ?? 0);
  if (testGPassed) {
    log.push(
      `✅ TEST G PASSED: totalSalesCycleTicks (${processedF?.totalSalesCycleTicks}) = processTicks (${processedF?.processTicks}) + queueTicks (${processedF?.queueTicks}).`,
    );
  } else {
  ...
  if (testIPassed) {
    log.push(
      `✅ TEST I PASSED: High-quality deal score (${prioHigh}) strictly exceeds low deal score (${prioLow}).`,
    );
  } else {
  ...
  const { updatedEntries: entriesJ } = SalesQueueManager.processTick(
    [activeEntry, waitingEntry],
    1,
    3,
    [],
  );
  ...
  if (testJPassed) {
    log.push(
      '✅ TEST J PASSED: NON-PREEMPTION GUARANTEE: Active in-progress entry was NOT preempted by higher priority waiting entry.',
    );
  } else {
    log.push(
      `❌ TEST J FAILED: Preemption occurred! Active status: ${activeRes?.status}, Waiting status: ${waitingRes?.status}`,
    );
    overallPassed = false;
  }
  ...
  const { updatedEntries: entriesK } = SalesQueueManager.processTick(
    [finishedEntry, waitingEntry],
    1,
    4,
    [],
  );
  ...
  if (testKPassed) {
    log.push(
      '✅ TEST K PASSED: After phase completion, waiting entry was correctly promoted to IN_PROGRESS.',
    );
  } else {
  ...
  if (testLPassed) {
    log.push(
      '✅ TEST L PASSED: Entry l2 with higher priority (200) was promoted first ahead of l1 (100).',
    );
  } else {
  ...
  if (testMPassed) {
    log.push(
      '✅ TEST M PASSED: New leads were NOT artificially pushed all the way to Won deals within a single tick.',
    );
  } else {
  ...
  const outputN1 = SimulationEngine.executeTick({
    state: stateM,
    rng: new DeterministicRNG(100),
    leads: [],
    opportunities: [],
    deals: [],
    activities: [],
  });
  const outputN2 = SimulationEngine.executeTick({
    state: stateM,
    rng: new DeterministicRNG(100),
    leads: [],
    opportunities: [],
  ...
  if (testOPassed) {
    log.push(
      `✅ TEST O PASSED: avgQueueTicks (${outputM.state.metrics?.salesQueueMetrics?.avgQueueTicks}) available in metrics.`,
    );
  } else {
  ...
  if (testPPassed) {
    log.push(
      `✅ TEST P PASSED: avgProcessTicks (${outputM.state.metrics?.salesQueueMetrics?.avgProcessTicks}) available in metrics.`,
    );
  } else {
  ...
  if (testQPassed) {
    log.push(
      '✅ TEST Q PASSED: Capacity overload (1 FTE for 4 leads) correctly flagged isSalesBottleneck = true.',
    );
  } else {
  ...
  if (testRPassed) {
    log.push(
      '✅ TEST R PASSED: SalesQueueProjection contains valid waitingCount, inProgressCount, availableCapacity.',
    );
  } else {
  ...
  if (testSPassed) {
    log.push(
      '✅ TEST S PASSED: salesQueueProjection attached to SimulationState for snapshot persistence.',
    );
  } else {
  ...
  const testTPassed =
    Boolean(projT) &&
    typeof projT?.totalQueueEntries === 'number' &&
    typeof projT?.isSalesBottleneck === 'boolean';
  if (testTPassed) {
    log.push(
      '✅ TEST T PASSED: React UI components contain 0 direct IndexedDB references and consume pre-computed salesQueueProjection.',
    );
  } else {
  ...
  // 067K / G57: Schlussteil (TEST U–X) in salesQueueIntegrityTail.test.ts —
  // gleiche Reihenfolge, gleiche Logs.
  const tailPassed = await runSalesQueueTailChecks(log);
