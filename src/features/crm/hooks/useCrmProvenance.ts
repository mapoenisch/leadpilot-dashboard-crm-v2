...
export function getCrmSubViewQueryKeyPrefix(activeSubView: string): readonly unknown[] {
...
export function useCrmProvenance(activeSubView: string = 's-leads'): CrmProvenanceResult {
  ...
  const deriveCurrentState = React.useCallback((): CrmProvenanceResult => {
    ...
    // 1. Spezifische Abfrage für aktive Unterseite suchen
    let queries = queryCache.findAll({ queryKey: prefix });
    // 2. Fallback auf allgemeine CRM-Abfragen, falls Subview noch nicht registriert
    if (queries.length === 0) {
      queries = queryCache.findAll({ queryKey: ['crm'] });
    }
