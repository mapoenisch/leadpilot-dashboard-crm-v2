...
describe('v2.3.0 data and simulation findings', () => {
  it('[PR-SOURCE-04] kennzeichnet jede CRM-Antwort mit Quellen-Envelope', async () => {
    ...
    expect
      .soft(envelope?.mode, 'Envelope nennt live/demo/baseline')
      .toMatch(/^(live|demo|baseline)$/);
  });
  ...
  it('[PR-PERSIST-08] persistiert Szenarien und Runs reload-fähig', () => {
    ...
    expect
      .soft(repositorySource, 'Speicherung erfolgt ausschließlich per Map.set')
      .toContain('.set(');
    expect
      .soft(repositorySource, 'Reload-fähiges Repository (Serialisierungs-/Lade-API oder Backend)')
      .toMatch(
