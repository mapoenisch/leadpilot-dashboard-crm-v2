#!/usr/bin/env node
/**
 * Sidebar-Footer-Version (PR #36): Screenshot-Gate für die Sidebar mit Footer.
 * Abgeleitet von scripts/captureAuftrag068Screenshots.mjs, aber nur die Sidebar-
 * Aufnahme: Login und Inhaltsseiten ändern sich durch den Footer-Text nicht.
 *
 * Erzeugt Screenshots auf 1440/768/375 px (unter 1024 px mit geöffnetem Drawer),
 * misst horizontalen Überlauf und schreibt SHA-256-Hashes nach
 * docs/screenshots/sidebar-footer-version/<label>/manifest.json. Bilder bleiben lokal
 * (.gitignore), nur die Ergebnis-Matrix wird committet.
 *
 * Aufruf: node scripts/captureSidebarFooterScreenshots.mjs <label>   (z. B. before | after)
...
const OUT_DIR = path.join(ROOT, 'docs/screenshots/sidebar-footer-version', LABEL);
...
async function main() {
  ...
  const beforeFile = path.join(
    ROOT,
    'docs/screenshots/sidebar-footer-version/before/manifest.json',
  );
  if (LABEL !== 'before') {
    if (!fs.existsSync(beforeFile)) {
      console.error('Vorher-Manifest fehlt: zuerst mit Label "before" gegen main laufen lassen.');
      process.exit(1);
    }
