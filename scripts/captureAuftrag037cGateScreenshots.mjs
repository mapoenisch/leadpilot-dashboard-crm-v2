...
const SCREENSHOT_DIR = path.resolve(process.cwd(), 'docs/screenshots/auftrag-037c');
fs.mkdirSync(SCREENSHOT_DIR, { recursive: true });

export const TARGET_ROUTES = [
  { id: 'profile', path: '/company/profile', name: 'Unternehmenssteckbrief', focusedSelector: '[data-testid="company-profile-matrix"]' },
  { id: 'highlights', path: '/company/highlights', name: 'Jahres-Highlights 2025', focusedSelector: '[data-testid="year-highlights-deck"]' },
  { id: 'data-basis', path: '/company/data-basis', name: 'Datenbasis & Konsistenz', focusedSelector: '[data-testid="data-basis-flow"]' },
  { id: 'features', path: '/product/features', name: 'Produkt & Funktionsweise', focusedSelector: '[data-testid="product-features-grid"]' },
  { id: 'pricing', path: '/product/pricing', name: 'Preismodell & Editionen', focusedSelector: '[data-testid="product-pricing-deck"]' },
  { id: 'performance', path: '/product/performance', name: 'Produkt-Performance 2025', focusedSelector: '[data-testid="product-performance-cockpit"]' },
  { id: 'roadmap', path: '/product/roadmap', name: 'Releases & Roadmap', focusedSelector: '[data-testid="product-roadmap-scene"]' },
];
...
export async function main() {
  console.log('=======================================================');
  console.log(`🚀 STARTING AUFTRAG 037C SCREENSHOT HARNESS (STAGE: ${(stage || 'N/A').toUpperCase()})`);
  console.log('=======================================================');
  ...
  try {
    ...
    const previewPort = await findAvailablePort(4220, HOST);
    ...
    const cdpPort = await findAvailablePort(9260, HOST);
    userDataDir = path.resolve(process.cwd(), `.tmp-chrome-037c-${stage}`);
    ...
    for (const route of TARGET_ROUTES) {
      ...
      for (const vp of VIEWPORTS) {
        ...
        // Capture focused element screenshot on 1440px
        if (vp.name === '1440') {
          console.log(`--- Capturing focused desktop element for ${route.id} ---`);
          const boxRes = await cdp.send('Runtime.evaluate', {
            expression: `(() => {
              const el = document.querySelector('${route.focusedSelector}') || document.querySelector('main > div') || document.querySelector('main');
              if (!el) return null;
    ...
    console.log('\n🎉 ALL AUFTRAG 037C SCREENSHOTS CAPTURED SUCCESSFULLY!');
  } finally {
