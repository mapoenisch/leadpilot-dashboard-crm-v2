...
export const TARGET_PAGE = {
  id: 'dashboard',
  path: '/dashboard',
  name: 'Executive Dashboard (Ebene A & C)',
};
...
async function main() {
  console.log('=======================================================');
  console.log(`🚀 STARTING AUFTRAG 035 SCREENSHOT HARNESS (STAGE: ${stage.toUpperCase()})`);
  console.log('=======================================================');
  ...
  try {
    ...
    const previewPort = await findAvailablePort(4194, HOST);
    ...
    // Launch Chrome
    const cdpPort = await findAvailablePort(9232, HOST);
    userDataDir = path.resolve(process.cwd(), `.tmp-chrome-035-${stage}`);
    ...
    for (const vp of VIEWPORTS) {
      ...
      // Verify LiveKpiCard existence and dimensions for nachher stage
      if (stage === 'nachher') {
        const cardCheck = await cdp.send('Runtime.evaluate', {
          expression: `(() => {
            const el = document.querySelector('[data-testid="live-kpi-card"]');
            if (!el) return { exists: false, width: 0, height: 0, text: '' };
        ...
        const cardInfo = cardCheck.result.value;
        if (!cardInfo.exists || cardInfo.width <= 0 || cardInfo.height <= 0) {
          throw new Error(`LiveKpiCard (data-testid="live-kpi-card") not found or has non-positive dimensions at ${vp.name}px: ${JSON.stringify(cardInfo)}`);
        }
        console.log(`✅ LiveKpiCard verified at ${vp.name}px: width=${cardInfo.width.toFixed(1)}px, height=${cardInfo.height.toFixed(1)}px`);
      }
      ...
      // Determine full scrollable content height to capture complete page including LiveKpiCard
