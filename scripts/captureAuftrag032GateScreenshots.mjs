...
const TARGET_PAGES = [
  { id: 'leads', path: '/crm/leads', name: 'Leads & Kontakte' },
  { id: 'companies', path: '/crm/companies', name: 'Unternehmen (Accounts)' },
  { id: 'deals', path: '/crm/deals', name: 'Deal Pipeline' },
  { id: 'activities', path: '/crm/activities', name: 'Aktivitäten-Historie' },
];
...
async function main() {
  console.log('=======================================================');
  console.log(`🚀 STARTING AUFTRAG 032 SCREENSHOT HARNESS (STAGE: ${stage.toUpperCase()})`);
  console.log('=======================================================');
  ...
  const previewPort = await findAvailablePort(4186);
  ...
  const chromePort = await findAvailablePort(9246);
  const chromePath = findChromePath();
  const userDataDir = path.resolve(process.cwd(), `.chrome-cdp-profile-g16-${stage}-${Date.now()}`);
